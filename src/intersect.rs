...
impl<T> BedRecords<T> {
    ...
    pub fn push(&mut self, r: BedRecord<T>) {
        ...
        match self.is_sorted.get_mut(&chr) {
            Some(b) => {
                *b = false;
            } // when new record is added, make is_sorted false
            None => {}
        }
    }
...
pub fn intersect_wa<'a, T1, T2>(
    a: &'a mut BedRecords<T1>,
...
pub fn intersect_wawb<'a, T1, T2>(
    a: &'a mut BedRecords<T1>,
    b: &'a mut BedRecords<T2>,
) -> (
    Vec<String>,
    HashMap<String, Vec<(&'a BedRecord<T1>, &'a BedRecord<T2>)>>,
) {
    // make a vector
    let mut intersect_map: HashMap<String, Vec<(&'a BedRecord<T1>, &'a BedRecord<T2>)>> =
        HashMap::new();
    let mut intersect_chrs: Vec<String> = Vec::new();
...
pub fn intersect_a<'a, T1: Clone, T2>(
    a: &'a mut BedRecords<T1>,
    b: &'a mut BedRecords<T2>,
) -> BedRecords<T1> {
    // make a vector
    ...
    for chr in &(b.sorted_chrs) {
        ...
        for r in &(a.map[chr]) {
            n = 0;
            for i in tree.find(r.start..r.end) {
                tmp_v.push(SimpleRange {
                    s: get_larger(r.start, i.interval().start),
