...
impl FastaRecords {
    ...
    /// This a chr from a fasta file with the fai file
    pub fn from_file_one_chr(fpath: &str, chr: &str) -> FastaRecords {
        let fapath = PathBuf::from(fpath);
        if !fapath.exists() {
            panic!("fasta file does not exist.");
        }
        ...
        // add the target chr in FastaRecords
        match fasta.fai.map.get_mut(chr) {
            Some(r) => {
                let mut seq = String::new();
                seq.reserve(fasta.fai.map[chr].length as usize);
                fasta.map.insert(chr.to_string(), seq);
            }
            None => {
                panic!("chr '{}' was not found in .fai file.", chr);
            }
        }
        // read fasta
        let f = File::open(fpath).expect("fai file not found.");
        ...
        reader
            .seek(SeekFrom::Current(
                fasta.fai.map[chr].offset.try_into().unwrap(),
            ))
            .expect("Seek failed!");
            ...
            match line {
                Ok(mut line) => {
                    if line.starts_with('>') {
                        ...
                        match fasta.map.get_mut(chr) {
                            Some(seq) => {
                                seq.push_str(line.trim());
                            }
                            None => {
                                panic!("{} does not found in fai", chr);
                            }
                        }
                    }
                }
    ...
    pub fn from_file_wo_fai(fpath: &str) -> FastaRecords {
        ...
        for chr in &(fasta.fai.chrs) {
            let mut r = FaiRecord::new(
                chr.clone().to_string(),
                fasta.map[chr].len() as u64,
                0,
                0,
                0,
            );
...
mod tests {
    ...
    fn test_load_fasta() {
        ...
        let path = "./test_files/test.fa".to_string();
        let mut fa = FastaRecords::from_file_one_chr(&path, "chr2");
        fa.to_upper();
        println!("{:?}", fa);
        println!("seq of only chr2: {}", fa.get_seq("chr2").unwrap());
    }
} // mod tests
