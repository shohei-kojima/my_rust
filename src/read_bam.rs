...
pub fn read_bam(f: &str) {
        ...
        .map(|x| x.expect("Failure parsing Bam file"))
    {
        ...
        if map_start < map_end {
            ...
            // println!("start: {}: {}", read.pos(), crate::utils::type_of(read.pos()));  // i32
            // println!("flag: {}: {}", read.flags(), crate::utils::type_of(read.flags()));  // u16
            // println!("qual: {:?}", read.qual());
            // println!("mean qual: {:?}", calc_mean_read_quality(read.qual()));
            // println!();

            match read.aux(b"SA") {
                Ok(v) => {
                    println!("{:?}, {}", v, crate::utils::type_of(&v));
                    if let rust_htslib::bam::record::Aux::String(sp) = v {
                        println!("{:?}", sp);
                    }
                }
                Err(e) => println!("{:?}", e),
...
fn calc_mean_read_quality(arr: &[u8]) -> usize {
...
mod tests {
    ...
    #[test]
    fn test_bam() {
        let bam_path = "/home/kooojiii/Documents/testdata/bams/1kgp/GRCh38DH/NA12878.final.bam";
        read_bam(bam_path);
    }
} // mod tests
